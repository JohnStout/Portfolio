...
LFP_concatenated = vertcat(LFP_trials{:}); % concatenating

% define some parameters
params.tapers     = [5 9];
params.trialave   = 0;       % everything same size
params.err        = [2 .05];
params.pad        = 0;
params.fpass      = [0 100]; 
...
% heatmaps
[heatmap_data,time,frex]=mtspecgramc(LFP_trials{1},params.movingwin,params);

frex_get = find(frex>30 & frex<100);
figure('color','w')
    pcolor(time,frex(frex_get),log10(heatmap_data(:,frex_get))');
    set(gca,'fontsize', 13);
    colormap(jet)
    colorbar   
    shading 'interp'
...
% make a plot that shows power across time    
frex_get = find(frex>4 & frex<12);

% isolate theta frequencies, and average across
theta_all = heatmap_data(:,frex_get);
theta_avg = mean(theta_all,2);

% plot theta average across time
figure('color','w')
plot(time,theta_avg,'k')

% plot theta average across all trials
for i = 1:length(LFP_trials)
    [power_trials{i},time_trials{i},frex_trials{i}]=mtspecgramc(LFP_trials{i},params.movingwin,params);
end
